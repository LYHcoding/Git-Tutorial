...
<table>
    ...
    <tr>
        ...
        <td>1. 查看指定配置项：<strong>git config &lt;parameter-name&gt;</strong>，例如：git config user.name。<br />2. 区分层级查看配置项：<strong>git config [--global/--system/--local] --list</strong>，例如查看全局配置项：git config --global --list，其他“system”表示需要管理员权限的系统配置项，“local”表示当前默认仓库配置。</td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td>1. 直接执行<strong>git init</strong>：在当前目录初始化仓库。<br />2. 执行<strong>git init my-project</strong>：创建名为my-project的目录并在其中初始化仓库。<br />3. 初始化后会生成隐藏的<strong>.git</strong>目录，包含仓库的所有配置和版本信息。</td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td>示例：<strong>git remote remove origin</strong>，仅删除本地关联，不影响远程仓库本身。</td>
    </tr>
    <tr>
        <td>git remote rename &lt;old-name&gt; &lt;new-name&gt;</td>
        <td>重命名已关联的远程仓库别名</td>
        <td>示例：<strong>git remote rename origin my-github</strong>，将默认的origin重命名为my-github。</td>
    </tr>
    <tr>
        <td>git remote set-url &lt;remote-alias&gt; &lt;new-url&gt;</td>
        <td>修改已关联远程仓库的URL地址</td>
        <td>1. 示例：<strong>git remote set-url origin git@github.com:xxx/xxx.git</strong>；<br />2. 适用场景：远程仓库地址变更、HTTPS切换为SSH协议等。</td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td>1. 可简化为“<strong>git pull</strong>”拉取跟踪的分支。<br />2. 本质是<strong>git fetch + git merge</strong>的组合命令。</td>
    </tr>
    <tr>
        <td>git pull --no-rebase [origin main]</td>
        <td>拉取远程分支并以merge方式合并（禁用rebase模式）</td>
        <td>1. --no-rebase：强制使用merge（默认行为，可显式指定）。<br />2. 对比git pull --rebase：rebase是“变基”合并，无多余合并提交。<br />3. 示例：<strong>git pull --no-rebase origin main</strong>（拉取远程main并merge）。</td>
    </tr>
    <tr>
        ...
        <td>1. 可简化为“<strong>git push</strong>”推送到跟踪分支。<br />2. 首次推送到远程并建立关联“<strong>git push -u origin &lt;branch-name&gt;</strong> ”。</td>
    </tr>
    <tr>
        ...
        <td>0. 默认使用git fetch origin <branch-name>。<br />1. 执行<strong>git fetch</strong>：拉取所有远程分支的最新更改。<br />2. 与git pull的区别：fetch只下载不合并，pull是下载+合并，fetch更安全，可先查看更改再决定是否合并。<br />3. 查看fetch的更改：<strong>git diff &lt;local-branch&gt; origin/&lt;remote-branch&gt;</strong>。</td>
    </tr>
    <tr>
    ...
    <tr>
        ...
        <td>对比两个分支之间的文件差异</td>
        <td>1. 示例：<strong>git diff main feature/login</strong>，查看main和feature/login分支的差异。<br />2. 查看fetch操作本地分支与远程分支间的更改：git diff &lt;local-branch&gt; origin/&lt;remote-branch&gt;。</td>
    </tr>
    <tr>
        <td align="center" rowspan="2">15</td>
        <td rowspan="2">git stash</td>
        <td>——</td>
        <td>暂存当前工作区/暂存区的未提交更改，恢复干净工作区</td>
        <td>1. 可选备注：<strong>git stash "临时修改备注"</strong>。<br />2. 暂存未跟踪文件需加<strong>-u</strong>参数。</td>
    </tr>
    <tr>
        <td>git stash pop</td>
        <td>恢复最近一次stash的更改，并删除该临时快照</td>
        <td>仅恢复不删除可用：<strong>git stash apply</strong>。</td>
    </tr>
    <tr>
        <td align="center" rowspan="2">16</td>
        <td rowspan="2">git log</td>
    ...
    <tr>
        ...
        <td>1. 区别于git reset：revert是“反向提交”，保留历史记录；reset是“删除提交”，修改历史。<br />2. 适用场景：撤销已推送到远程的提交（不破坏公共历史）。<br />3. 示例：<strong>git revert abc1234</strong>，撤销abc1234提交的修改。<br />4. 若有冲突，需解决后执行git revert --continue。</td>
    </tr>
</table>
